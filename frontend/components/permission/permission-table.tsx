...
export default function RoleTable({ permissions, onCreate, onEdit, onDelete, onOpenPermissionModal, onView }: Props) {
  ...
  // Role icon colors — cycle through a few green/cyan tints
  const avatarColors = [
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr>
                ...
                >
                  Permission Name <SortIcon active dir={sortDir} />
                </th>
            ...
            <tbody>
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ padding: "60px 24px", textAlign: "center", color: "#94a3b8" }}>
                    <div style={{ fontSize: 32, marginBottom: 10 }}>🔐</div>
                    <p style={{ fontWeight: 600, color: "#64748b", margin: "0 0 4px" }}>No permissions found</p>
                    <p style={{ fontSize: 13, margin: 0 }}>Try adjusting your search or add a new role</p>
                  </td>
                </tr>
              ) : paginated.map((role, i) => {
                const isHovered = hoveredRow === role._id;
                ...
                return (
                  <tr
                    key={role._id}
                    onMouseEnter={() => setHoveredRow(role._id)}
                    ...
                    <td style={{ padding: "16px 18px", textAlign: "right" }}>
                      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
                        ...
                        <button
                          ...
                          disabled={isSystem}
                          title={isSystem ? "System permissions cannot be deleted" : undefined}
                          style={{
                            background: "transparent",
                            border: `1.5px solid ${isSystem ? "#e2e8f0" : "#fee2e2"}`,
                            borderRadius: 8, padding: "6px 14px",
                            fontSize: 12, fontWeight: 600,
                            color: isSystem ? "#cbd5e1" : "#dc2626",
                            cursor: isSystem ? "not-allowed" : "pointer",
                            transition: "all .15s",
                            display: "flex", alignItems: "center", gap: 5,
                            opacity: isSystem ? 0.5 : 1
                          }}
                          onMouseEnter={e => { if (!isSystem) { e.currentTarget.style.background = "#fef2f2"; e.currentTarget.style.borderColor = "#fca5a5"; } }}
                          onMouseLeave={e => { if (!isSystem) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "#fee2e2"; } }}
                        >
