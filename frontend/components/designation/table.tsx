...
export default function Table({ designation, onCreate, onEdit, onDelete }: Props) {
              ...
              {paginated.length === 0 ? (
                <tr>
                  <td colSpan={5} style={{ padding: "60px 24px", textAlign: "center", color: "#94a3b8" }}>
                    ...
                    <p style={{ fontWeight: 600, color: "#64748b", margin: "0 0 4px" }}>No designation found</p>
                    <p style={{ fontSize: 13, margin: 0 }}>Try adjusting your search or add a new designation</p>
                  </td>
                </tr>
              ) : paginated.map((designation, i) => {
                const isHovered = hoveredRow === designation._id;
                const isLast = i === paginated.length - 1;
                const avatar = avatarColors[i % avatarColors.length];
                const globalIndex = (page - 1) * pageSize + i + 1;
                const isActive = designation.status === true || (designation.status as any) === "true";

                return (
                  <tr
                    key={designation._id}
                    onMouseEnter={() => setHoveredRow(designation._id)}
                    ...
                    <td style={{ padding: "16px 18px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{
                          ...
                          {designation.title?.charAt(0).toUpperCase()}
                        </div>
