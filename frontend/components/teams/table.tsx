...
interface Props {
  ...
  loading?: boolean;
  onCreate: () => void;
  onEdit?: (teams: Team) => void;  
    // onView → onEdit
  onDelete?: (teams: Team) => void;
}
...
export default function Table({ teams, loading, onCreate, onEdit, onDelete }: Props) {
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                const isHovered = hoveredRow === team._id;
                const isLast = i === paginated.length - 1;
                const avatar = avatarColors[i % avatarColors.length];
                const globalIndex = (page - 1) * pageSize + i + 1;
                const isActive = team.status === true || (team.status as any) === "true";

                return (
                  <tr
                    key={team._id}
                    onMouseEnter={() => setHoveredRow(team._id)}
