...
export default function EmployeeSalaryModal({
  open, mode, employeeName, form, setForm, onClose, onSubmit,
}: Props) {
  if (!open) return null;
    ...
    >
      <div style={{
        ...
        <ModalHeader
          ...
          subtitle={`Configure earnings, deductions & fines for ${employeeName}`}
          onClose={onClose}
        />
        ...
        <div style={{ padding: "24px 28px", maxHeight: "68vh", overflowY: "auto",
          ...
          {/* ── Fines ── */}
          <div>
            <SectionLabel title="Fines & Charges" color="#d97706" />
            <p style={{ fontSize: 12, color: "#9ca3af", marginBottom: 10 }}>
              Late arrival, absenteeism, damage, misconduct penalty…
            </p>
            <LineItemList
              items={form.fines}
              accentColor="#d97706"
              placeholder="e.g. Late Fine"
              presets={["Late Arrival Fine", "Absence Deduction", "Damage Charge",
                        "Misconduct Penalty", "Uniform Charge", "ID Card Charge"]}
              onChange={(id, field, val) => updateItems("fines", id, field, val)}
              onAdd={() => addItem("fines")}
              onRemove={(id) => removeItem("fines", id)}
            />
          </div>
